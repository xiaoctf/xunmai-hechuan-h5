...
新版内容主线：以合川桃片、合川肉片双非遗美食为流量载体，带动峡砚、泥塑、木雕等小众非遗传播，联动非遗数字化二维码 H5 档案、桃片&肉片联合礼盒礼袋、合川非遗冰箱贴系列、非遗主题三下乡短视频、市场调研数据、扫码即购入口与落地建议五类成果。
...
- 对接渠道：购买点击事件会带上 `product_id`、`product_name`、`product_channel`，便于后台区分桃片、肉片及对应渠道。
- 受访页面兜底：若百度统计“事件分析”无权限，可在 `访问分析 -> 受访页面` 搜索 `/xunmai-event/` 查看真实统计页计数。
...
5. 如果账号不能查看“事件分析”，可改看百度统计 `访问分析 -> 受访页面` 中的真实统计页：
   - `/xunmai-event/product-section-view/`：产品区曝光。
   - `/xunmai-event/purchase/hechuan-taopian/`：合川桃片购买入口点击。
   - `/xunmai-event/purchase/hechuan-roupian/`：合川肉片购买入口点击。
